...
if [ -r "$FILEINFO" ]; then
	...
	case ${REPO} in
        GIT)
			...
			URLGIT=`echo ${URL} | sed -e 's/http\:\/\//http\:\/\/testgit\:xxxxxxxxxxxxx\@/g'` 
			git init ${DIRKW}
			git --version 
			PWDBCK=`pwd`
			cd ${DIRKW}
        ...
        SVN)
			...
			if  echo "${VERS}" | grep -q -i "SNAPSHOT"; then
				...
				svn export --quiet --force --username testsvn --password xxxxxxxx --non-interactive ${URL} ${DIRKW}
				EXIT_S="$?"
			else # si es una release, debe de estar en un tag
				URLSVN=`echo ${URL} | sed -e 's/trunk/tags/g'` 
				log "Release detectada [${URLSVN}/${VERS}]"
				svn export --quiet --force --username testsvn --password xxxxxxxxxxxxx --non-interactive ${URLSVN}/${VERS} ${DIRKW}
				EXIT_S="$?"
			fi 
