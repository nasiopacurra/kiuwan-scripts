...
function f_CargaVars { 
	...
	# USERKW=
	# PASSKW=
	. /mma/datos/scripts/_kiuwan.properties
	cd ${PWD_backup}
...
function f_crearApplVacia {
	...
	PARMS="${PARMS} -s /mma/datos/scripts/helloWorld"
...
FILEINFO=$1
if [ -r "${FILEINFO}" ] && [ -f "/mma/datos/scripts/_kiuwan.properties" ]; then
	...
	PWD_backup=`pwd`
	cd /mma/datos/scripts
	bAppl=`./EXIST_application_Kiuwan.php ${APPL}`
	cd ${PWD_backup}
	if [ "${bAppl}" == "false" ]; then 
