...
if [ -r "$FILEINFO" ]; then
	...
	case ${REPO} in
        GIT)
			...
			URLGIT=`echo ${URL} | sed -e 's/http\:\/\//http\:\/\/testgit\:xxxxxxxxxxxxxx\@/g'` 
			git init ${DIRKW}
			git --version 
			PWDBCK=`pwd`
			cd ${DIRKW}
        ...
        SVN)
			...
			if  echo "${VERS}" | grep -q -i "SNAPSHOT"; then
				...
				svn export --quiet --force --username testsvn --password xxxxxxxxxxxxxx --non-interactive ${URL} ${DIRKW}
				EXIT_S="$?"
